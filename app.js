...
const key='l2one_state_v2', defaults={clients:[],visits:[],orders:[],tasks:[],goals:[],routes:[],prices:[],officeProcesses:[],officeActions:[],officeCommercial:[],officeAdministrative:[],officeRituals:[],officeRoles:[],officeFinance:[],officeBudget:[],officeMonthlyClose:[],cashDays:[],cashEntries:[],user:'Ana Paula',server:'',token:'',mustChangePassword:false,syncAt:'',pending:[]};
...
if(tab==='marcas'){let prices=(s.prices||[]).slice().sort((a,b)=>[a.brand,a.state,a.description,a.sku].join('|').localeCompare([b.brand,b.state,b.description,b.sku].join('|'),'pt-BR')),brands=[...new Set(prices.map(p=>p.brand).filter(Boolean))];h=`<div class="page-heading"><div><span class="eyebrow">Catálogo comercial</span><h2>Marcas e preços</h2><p>Consulte os valores oficiais por estado. Todos os preços são exibidos em Real.</p></div></div><div class="cards"><div class="card">Marcas<strong>${brands.length}</strong></div><div class="card">Produtos<strong>${prices.length}</strong></div><div class="card">Tabela PA<strong>${prices.filter(p=>p.state==='PA').length}</strong></div><div class="card">Tabela AP<strong>${prices.filter(p=>p.state==='AP').length}</strong></div></div>${s.user==='Ana Paula'?`<div class="box"><h3>Importar tabela de preços</h3><p class="muted">Uma linha por produto no formato: Marca; SKU; Descrição; UF; Preço. Exemplo: Bruna Tavares; BT001; Batom; PA; 49,90</p><form id="priceImportForm"><textarea name="prices" rows="7" placeholder="Marca;SKU;Descrição;UF;Preço" required></textarea><p><button>Importar e substituir os SKUs informados</button></p></form></div>`:''}<div class="box"><h3>Tabela cadastrada</h3>${prices.length?`<div class="scroll"><table><tr><th>Marca</th><th>SKU</th><th>Produto</th><th>UF</th><th>Preço</th></tr>${prices.map(p=>`<tr><td><b>${esc(p.brand)}</b></td><td>${esc(p.sku)}</td><td>${esc(p.description||'—')}</td><td><span class="uf-badge">${esc(p.state)}</span></td><td><b>${money(p.price)}</b></td></tr>`).join('')}</table></div>`:'Nenhuma tabela de preços foi importada. A administradora pode usar o formulário acima.'}</div>`}
...
if(tab==='registro'){let id=window.prefillId||'';h=`<div class="page-heading"><div><span class="eyebrow">Relacionamento</span><h2>Registrar atendimento</h2><p>Registre a visita e as próximas ações. Para vender, use a aba Pedidos.</p></div></div><form id="visitForm" class="box"><div class="row"><label>Data<input name="date" type="date" value="${today()}" required></label><label>Cliente<select name="clientId" required><option value="">Selecione</option>${s.clients.map(c=>`<option value="${esc(c.id)}" ${c.id===id?'selected':''}>${esc(c.name)}</option>`).join('')}</select></label><label>Resultado<select name="result">${options(['Visitado - pedido','Visitado - sem pedido','Não visitado','Reagendado','Contato remoto'],'Visitado - pedido')}</select></label><label>Próxima ação<input name="next" placeholder="Ex.: Enviar proposta"></label><label>Data de retorno<input name="returnDate" type="date"></label></div><label>Observações<textarea name="notes" rows="3"></textarea></label><p><button>Salvar atendimento</button> <button type="button" class="secondary" onclick="show('pedidos')">Ir para pedidos</button></p></form><div class="box"><h3>Últimos atendimentos</h3>${s.visits.slice(-12).reverse().map(v=>`<p>${esc(v.date)} · ${esc(client(v.clientId).name)} · ${esc(v.result)} · ${esc(v.user)}</p>`).join('')}</div>`}
if(tab==='pedidos'){let orders=s.orders.slice().reverse(),draft=orderDraft(),catalog=pricesForTable(draft.priceTable),brands=[...new Set(catalog.map(p=>p.brand))].sort((a,b)=>a.localeCompare(b,'pt-BR')),products=catalog.filter(p=>p.brand===draft.brand);h=`<div class="page-heading"><div><span class="eyebrow">Venda com preço oficial</span><h2>Pedidos</h2><p>Escolha a tabela de preços, a marca e as quantidades. Todos os valores são calculados em Real.</p></div><button onclick="newOrder()">Novo pedido</button></div><div class="cards"><div class="card">Pedidos<strong>${orders.length}</strong></div><div class="card">Valor total<strong>${money(orders.reduce((a,x)=>a+Number(x.amount),0))}</strong></div><div class="card">Faturado<strong>${money(orders.filter(x=>x.status==='Faturado').reduce((a,x)=>a+Number(x.amount),0))}</strong></div></div><form id="orderForm" class="box"><h3>Novo pedido</h3><div class="row"><label>Data<input name="date" type="date" value="${esc(draft.date)}" onchange="setOrderDate(this.value)" required></label><label>Cliente<select name="clientId" onchange="setOrderClient(this.value)" required><option value="">Selecione</option>${s.clients.map(c=>`<option value="${esc(c.id)}" ${c.id===draft.clientId?'selected':''}>${esc(c.name)} · ${esc(stateCode(c)||'UF pendente')}</option>`).join('')}</select></label><label>Tabela de preços<select name="priceTable" onchange="setOrderTable(this.value)" required><option value="">Selecione</option><option value="PA" ${draft.priceTable==='PA'?'selected':''}>Tabela Pará (PA)</option><option value="AP" ${draft.priceTable==='AP'?'selected':''}>Tabela Amapá (AP)</option></select></label><label>Marca<select name="brand" onchange="setOrderBrand(this.value)" required><option value="">${draft.priceTable?'Selecione':'Escolha a tabela primeiro'}</option>${brands.map(b=>`<option value="${esc(b)}" ${b===draft.brand?'selected':''}>${esc(b)}</option>`).join('')}</select></label><label>Status<select name="status">${options(['Pendente','Confirmado','Faturado'],'Pendente')}</select></label></div>${draft.priceTable&&!catalog.length?'<p class="warn">Não há preços cadastrados nesta tabela. Consulte a aba Marcas e preços.</p>':''}${products.length?`<p class="table-choice">Usando <b>Tabela ${esc(draft.priceTable)}</b> · <b>${esc(draft.brand)}</b></p><div class="scroll"><table class="order-items"><tr><th>Produto</th><th>SKU</th><th>Preço unitário</th><th>Quantidade</th><th>Subtotal</th></tr>${products.map(p=>{let qty=Number(draft.quantities[p.sku]||0),subtotal=priceCents(p.price)*qty;return `<tr><td><b>${esc(p.description||p.sku)}</b></td><td>${esc(p.sku)}</td><td>${money(priceCents(p.price)/100)}</td><td><input aria-label="Quantidade ${esc(p.sku)}" type="number" min="0" max="100000" step="1" value="${qty||''}" oninput="setOrderQuantity('${esc(p.sku)}',this.value)"></td><td data-subtotal="${esc(p.sku)}">${money(subtotal/100)}</td></tr>`}).join('')}</table></div><div class="order-total"><span>Total exato do pedido</span><strong id="orderTotal">${money(orderTotalCents(products,draft)/100)}</strong></div><p><button name="action" value="draft">Salvar rascunho</button> <button class="secondary" name="action" value="finalize">Finalizar e gerar PDF</button></p>`:(draft.brand?'<p class="warn">Esta marca ainda não possui produtos na tabela escolhida.</p>':'<p class="muted">Escolha cliente, tabela de preços e marca para exibir os produtos.</p>')}</form><div class="box"><h3>Pedidos registrados</h3>${orders.length?`<div class="scroll"><table><tr><th>Data</th><th>Cliente</th><th>Tabela</th><th>Marca</th><th>Itens</th><th>Valor</th><th>Status</th><th>Usuário</th><th>Documento</th></tr>${orders.slice(0,250).map(x=>`<tr><td>${esc(x.date)}</td><td>${esc(client(x.clientId).name)}</td><td><span class="uf-badge">${esc(x.priceTable||x.state||'—')}</span></td><td>${esc(x.brand)}</td><td>${Array.isArray(x.items)?x.items.reduce((n,i)=>n+Number(i.quantity||0),0):'—'}</td><td><b>${money(x.amount)}</b></td><td><select onchange="changeOrder('${x.id}',this.value)">${options(['Pendente','Confirmado','Faturado','Cancelado'],x.status)}</select></td><td>${esc(x.user)}</td><td><button type="button" onclick="downloadOrderPDF('${x.id}')">PDF</button> <button type="button" class="secondary" onclick="shareOrderPDF('${x.id}')">WhatsApp</button></td></tr>`).join('')}</table></div>`:'Nenhum pedido registrado.'}</div>`}
...
if(tab==='office'){let section=window.officeSection||'painel',buttons=[['painel','Painel'],['processos','Processos'],['acoes','Plano de ação'],['comercial','Comercial'],['administrativo','Administrativo']];let sub=`<div class="subtabs">${buttons.map(([k,l])=>`<button class="${section===k?'active':''}" onclick="window.officeSection='${k}';show('office')">${l}</button>`).join('')}</div>`;if(section==='painel'){let byArea={};(s.officeProcesses||[]).forEach(p=>byArea[p['Área']]=(byArea[p['Área']]||0)+1);h=`<div class="page-heading"><div><span class="eyebrow">Gestão integrada</span><h2>Escritório</h2><p>Processos, ações, comercial e administrativo em uma única visão.</p></div></div>${sub}<div class="cards"><div class="card">Processos ativos<strong>${(s.officeProcesses||[]).length}</strong></div><div class="card">Planos de ação<strong>${(s.officeActions||[]).length}</strong></div><div class="card">Demandas comerciais<strong>${(s.officeCommercial||[]).length}</strong></div><div class="card">Pendências administrativas<strong>${(s.officeAdministrative||[]).length}</strong></div></div><div class="box"><h3>Processos por área</h3>${Object.entries(byArea).map(([k,v])=>`<p>${esc(k)}: <b>${v}</b></p>`).join('')||'Nenhum processo importado.'}</div><div class="box"><h3>Ritual de gestão</h3>${(s.officeRituals||[]).length?`<div class="scroll"><table><tr><th>Momento</th><th>Responsáveis</th><th>Saída obrigatória</th></tr>${s.officeRituals.map(r=>`<tr><td>${esc(r['Momento'])}</td><td>${esc(r['Responsáveis'])}</td><td>${esc(r['Saída obrigatória'])}</td></tr>`).join('')}</table></div>`:'Nenhum ritual importado.'}</div>`}if(section==='processos'){h=`<h2>Processos do escritório</h2>${sub}<div class="scroll"><table><tr><th>ID</th><th>Área</th><th>Processo</th><th>Responsável</th><th>Frequência</th><th>Prazo</th><th>Prioridade</th><th>Status</th></tr>${(s.officeProcesses||[]).map(p=>`<tr><td>${esc(p.ID)}</td><td>${esc(p['Área'])}</td><td><b>${esc(p.Processo)}</b><br><small>${esc(p['Objetivo/Entrega'])}</small></td><td>${esc(p['Responsável'])}</td><td>${esc(p['Frequência'])}</td><td>${esc(p['Prazo/SLA'])}</td><td>${esc(p['Prioridade'])}</td><td>${esc(p['Status do processo'])}</td></tr>`).join('')}</table></div>`}if(section==='acoes'){h=`<h2>Plano de ação</h2>${sub}<div class="box">${(s.officeActions||[]).length?`<div class="scroll"><table><tr><th>ID</th><th>Demanda</th><th>Responsável</th><th>Prazo</th><th>Status</th></tr>${s.officeActions.map(a=>`<tr><td>${esc(a.ID)}</td><td>${esc(a['Demanda ou problema'])}</td><td>${esc(a['Responsável'])}</td><td>${esc(a['Prazo'])}</td><td>${esc(a['Status'])}</td></tr>`).join('')}</table></div>`:'A estrutura da planilha foi importada. Ainda não existem ações preenchidas.'}</div>`}if(section==='comercial'){h=`<h2>Gestão comercial do escritório</h2>${sub}<div class="box">${(s.officeCommercial||[]).length?JSON.stringify(s.officeCommercial):'A estrutura comercial foi importada. Ainda não existem registros preenchidos na planilha.'}</div>`}if(section==='administrativo'){h=`<h2>Controle administrativo</h2>${sub}<div class="box">${(s.officeAdministrative||[]).length?JSON.stringify(s.officeAdministrative):'A estrutura administrativa foi importada. Ainda não existem registros preenchidos na planilha.'}</div>`}}
...
if(tab==='office'||tab==='finance'){
 ...
 const sectionMap={office:{processos:'officeProcesses',acoes:'officeActions',comercial:'officeCommercial',administrativo:'officeAdministrative'},finance:{lancamentos:'officeFinance',orcamento:'officeBudget',fechamento:'officeMonthlyClose'}};
